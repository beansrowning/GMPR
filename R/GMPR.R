#' @title Geometric Mean of Pairwise Ratios (GMPR)
...
GMPR <- function(OTUmatrix, min_ct = 2, intersect_no = 4) {
  ...
  OTUmatrix <- apply(OTUmatrix, MARGIN = 2, as.integer)

  size.factor <- gmpr(OTUmatrix, min_ct, intersect_no)
